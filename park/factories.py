...
class ParkFactory:
    ...
    def create_check_out_interator(id: Any):
        return CheckOutInterator(repo=ParkRepo, serializer=DefaultSerializer)\
                                                    .set_params(parking_id=id)\
                                                    .execute()

    @staticmethod
    def create_do_payment_interator(id: Any):
        return DoPaymentInterator(repo=ParkRepo, serializer=DefaultSerializer)\
                                            .set_params(parking_id=id)\
                                            .execute()

    @staticmethod
    def create_historic_interator(plate: str):
        return HistoricInterator(validator=ParkValidator,
                                 repo=ParkRepo,
                                 serializer=HistoricSerializer)\
